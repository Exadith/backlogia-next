...
def update_igdb(game_id: int, body: UpdateIgdbRequest, conn: sqlite3.Connection = Depends(get_db)):
    ...
    if igdb_id is None:
        ...
        return {
            "success": True,
            "message": "IGDB metadata and cached assets cleared"
        }
    ...
    try:
        ...
        if igdb_game.get("cover"):
            cover_url_from_igdb = igdb_game["cover"].get("url", "")
        ...
        # Extract screenshot URLs from IGDB
        screenshot_urls = []
        ...
        # Preserve existing Steam App ID
        cursor = conn.cursor()
        cursor.execute(
        ...
        row = cursor.fetchone()
        steam_app_id = row[0] if row else None
