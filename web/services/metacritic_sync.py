...
class MetacriticClient:
    ...
    def search_game(self, name):
        ...
        # Find search result items (new Metacritic layout)
        result_cards = soup.select("a.c-search-item[href*='/game/']")
        
        if not result_cards:
            # Fallback
            result_cards = soup.select("div.search-item a[href*='/game/']")
        ...
        for card in result_cards[:5]:  # Limit to top 5 results
            try:
                ...
                # Game title
                title_el = card.select_one(".c-search-item__title")
                game_name = (
                ...
                score = None
                score_el = (
                    card.select_one(".c-siteReviewScore span")
                    or card.select_one('[aria-label*="Metascore"] span')
                )
    ...
    def get_game_by_slug(self, slug):
        ...
        # ----------------------------------------------------------
        result["critic_score"] = None
        
        for score_block in soup.select(".c-siteReviewScore"):
            aria = (score_block.get("aria-label") or "").lower()
        
            if "metascore" not in aria:
                continue
        
            score_el = score_block.select_one("span")
            if not score_el:
                continue
        ...
        for future in as_completed(future_to_game):
            ...
            try:
                ...
                if success:
                    ...
                    score_str = ""
                    if result.get("critic_score") is not None:
                        score_str = f" (Critic: {result['critic_score']}"
                        if result.get("user_score") is not None:
